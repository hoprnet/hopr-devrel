...
sync_stake AS (
    SELECT synced.account, last_sync_time, now() AS now_utc, 
        ...
        793 AS actual_base -- define base (APR of 2.5%)
    FROM synced
...
rewards_with_projection AS (
    ...
    FROM (
        ...
        SELECT
            ((LEAST(GREATEST(now_epoch, basic_start), program_end)-LEAST(GREATEST(last_sync_time_epoch, basic_start), program_end)) * (actual_base * actual_stake + total_boost * LEAST(actual_stake, 2e5))) / 1e12 -- 200000 cap 
            + last_sync_reward AS rewards_till_now,
            ((LEAST(GREATEST(unix_timestamp(to_utc_timestamp('{{End of Estimation}}'::TIMESTAMP, 'UTC')), basic_start), program_end)-LEAST(GREATEST(last_sync_time_epoch, basic_start), program_end)) * (actual_base * actual_stake + total_boost * LEAST(actual_stake, 2e5))) / 1e12
            ...
            ((program_end-basic_start) * (actual_base * actual_stake)) / 1e12 AS allstaked_rewards_without_boost,
            ((program_end-basic_start) * (actual_base * actual_stake+ total_boost * LEAST(actual_stake, 2e5))) / 1e12 AS allstaked_rewards_with_boost,
            /* For this I need to figure out synced rewards without boost */ 
            --((LEAST(GREATEST(now_epoch, basic_start), program_end)-LEAST(GREATEST(last_sync_time_epoch, basic_start), program_end)) * (actual_base * actual_stake)) / 1e12  
            -- last_sync_reward_without_boost AS rewards_till_now_without_boost,
            * 
        FROM details
...
rewards_with_projection_adj AS (
    ...
    ROUND((actual_base / 1e12 * 60 * 60 * 24 * 365), 3) * 100 AS APR, -- convert APR into a percentage
    ROUND((total_boost / 1e12 * 60 * 60 * 24 * 365), 3) * 100 AS total_boost, -- convert Total boost into a percentage
    allstaked_rewards_without_boost, allstaked_rewards_with_boost,
    sqrt(actual_stake) AS voting_power 
    FROM rewards_with_projection)

SELECT * 
FROM rewards_with_projection_adj
WHERE actual_stake > 0
ORDER BY actual_stake DESC